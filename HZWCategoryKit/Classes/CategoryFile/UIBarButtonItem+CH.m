...
//    btn.titleEdgeInsets = UIEdgeInsetsMake(0, -10, 0, 0);
    ...
    btn.frame=CGRectMake(width, 20, btn.currentImage.size.width, btn.currentImage.size.height);
    [btn addTarget:target action:action forControlEvents:UIControlEventTouchUpInside];
    return [[UIBarButtonItem alloc]initWithCustomView:btn];
...
+(UIBarButtonItem*)itemWithRightTitle:(NSString*)title target:(id)target action:(SEL)action {
    ...
    CGFloat width = [UIScreen mainScreen].bounds.size.width;
    UIButton *btn=[[UIButton alloc]initWithFrame:CGRectMake(width - 20, 0, 50, 30)];
    btn.titleLabel.font=[UIFont systemFontOfSize:15];
    [btn addTarget:target action:action forControlEvents:UIControlEventTouchUpInside];
    [btn setTitle:title forState:UIControlStateNormal];
